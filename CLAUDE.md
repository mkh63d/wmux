...
| `cdp-proxy.ts` | CDP WebSocket proxy, plus the HTTP endpoints a CDP client probes before it opens one. The rule the route table has to be read against is that this proxy **impersonates Chrome**, so its contract is not "what wmux documented" but "what real Chrome accepts" — anything that attaches to Chrome must attach here. Exact-match on `req.url` quietly broke that (#233): Chrome serves `/json/version` and `/json/version/` alike, Playwright's `connectOverCDP` probes the TRAILING-SLASH form, and the 404 it got read as a Playwright bug because `curl /json/version` returned 200 right beside it. `cdpRoutePath` folds the spellings (and drops a query string, which was the same bug unreported) and is pure so the accepted table is testable without binding a port, like `isAllowedCdpHost`/`isAllowedCdpOrigin` above it. Normalization accepts only what Chrome accepts — an unknown route still 404s in both spellings, and neither guard is on this path. Since 2.12.0 it is also a BROWSER rather than a pipe (#237) — see `cdp-browser-domain.ts` |
| `cdp-browser-domain.ts` | The browser-level protocol, answered by wmux instead of forwarded (#237). The proxy used to hand every message to `webContents.debugger`, which is a PAGE-level debugger, so the first thing `puppeteer.connect({browserURL})` sends — `Target.getBrowserContexts` — came back "Not allowed" and no real client ever got a session; chrome-devtools-mcp listed its 29 tools and failed on the first one that touched the page. wmux has one page to offer, so being a browser is bookkeeping: one target, one flattened session, the default browser context, and a translation down to Electron's single unsessioned debugger. Pure like `agent-browser-verbs.ts`, for the same reason. **Two rules, both found by pointing real puppeteer at it rather than by reading it.** (1) A reply to a session-scoped command MUST echo its `sessionId` — puppeteer dispatches on `sessionId` before it looks at `id`, so dropping it resolves nothing and the caller hangs to its own timeout, which reads as a slow page. (2) `Target.*` on the PAGE session must emit NO events. When `setAutoAttach` there answered the way the browser connection does — with an `attachedToTarget` for the page — puppeteer read it as a newly discovered child, attached, sent `setAutoAttach` down the new session, and got another attachment for the same target: 4 GB heap and `Ineffective mark-compacts near heap limit` inside `browser.pages()`. A unit test asserting the return value passes. Single-pane concessions: `createTarget` navigates the pane and answers with the target already there, `closeTarget` is `success:false` rather than a lie, `createBrowserContext` is refused rather than silently sharing cookies, `Browser.close` is swallowed — that "browser" is the user's window |
...
| `agent-spawn-target.ts` | Which (pane, workspace) pair a spawn actually means (#242). They used to be resolved INDEPENDENTLY — `workspaceId` off the active workspace, `paneId` taken verbatim — so nothing ever checked that the pane belonged to the workspace it was filed under. A pane from a non-active workspace therefore produced a live, running agent recorded against the wrong workspace, after which every lookup through that record failed: `read-screen` answered "no terminal for surface … (markdown/browser pane, another window, or closed)", three causes none of which was the real one, and the reporter restarted three orchestration waves chasing it. The rule is that **the pane is authoritative whenever it is given** — a pane belongs to exactly one workspace and the split tree says which, so `--pane` alone reads as fully specified because it is. Both named and disagreeing is -32602, not a silently mismatched record; an unknown pane is -32602 rather than an agent nobody can address. With a pane named the active workspace is never consulted, so a fully-specified spawn now works when there is no active workspace at all. The MIRRORED half the report only suspected is here too: pane loads were always read off the ACTIVE workspace, so `spawn_batch --workspace <other>` honoured the flag for the record and ignored it for the panes. Pure, with the renderer lookups injected — the split tree lives in the Zustand store and main has no copy, so every question is a round trip, which is exactly what makes the decision worth separating from the asking. `spawnTargetLookups` in `index.ts` is the impure half and asks EVERY window, first real answer wins (#143) |
...
| `claude-context.ts` | Injects wmux instructions into `~/.claude/CLAUDE.md`, configures hooks, points chrome-devtools-mcp at the browser panel — **and the inverse of each**, since 0.40.0. Two things it no longer does, both because they were writing into files on terms Claude Code does not use. The orchestrator plugin is DEPRECATED and uninstall-only (#239): wmux hand-wrote `installed_plugins.json`, whose v2 shape is `{version:2, plugins:{"<plugin>@<market>":[…]}}`, as a bare object at the TOP level, so Claude Code listed nothing, loaded nothing and marked the copied tree `.orphaned_at` while wmux logged a success — and native parallel orchestration makes repairing it pointless. `removeOrchestratorPlugin` therefore runs in BOTH branches of the toggle, and must never remove `plugins[key]` or clear an `enabledPlugins` flag with a real install behind it (`orchestratorFlagIsStale`): that is where the hand-installed replacement lands. And the `chrome-devtools` MCP entry goes to `~/.claude.json`, NOT `~/.claude/settings.json` (#237) — settings.json has no `mcpServers` key and its schema strips unknown keys, so six releases' worth of writes landed, changed nothing, and logged success; the stale entry is swept on both paths. The per-tool-call hooks (PreToolUse, PostToolUse, UserPromptSubmit) are written with `async: true`: they are pure observers, and Claude Code waited a measured 125-145 ms on each of them on the critical path of every tool call. Ordering is already handled — every report carries the wall-clock it fired at and `agent-state.ts` drops anything older than what it accepted (#151). SessionStart, Stop and SessionEnd stay sync. `applyWmuxHooks` rewrites its own entries on every launch, so an install that predates the flag picks it up without a migration |
| `agent-integration.ts` | Consent gate for every write outside `%APPDATA%\wmux` (issue #132). Asks on first launch, stores `unset`/`granted`/`declined` in wmux's own settings.json, and reconciles `~/.claude` + `~/.config/opencode` + `~/.kiro` + `~/.omp` + `~/.pi` to match. Nothing in `claude-context.ts`, `opencode-context.ts`, `kiro-context.ts`, `omp-context.ts` or `pi-context.ts` may be called directly from startup any more — route it through here. Which FEATURE a new integration hangs off is a real decision, not a slot: the pi extension is under `hooks` because the panel's own label for that toggle is "Status hooks" and that is exactly what it is, while the OpenCode plugin does the same job from under `orchestrator` — a pre-existing mismatch left alone deliberately, since moving it would silently re-enable a status bridge for everyone who switched that toggle off |
...
| `opencode-context.ts` | Installs `resources/opencode-plugin/wmux.js` into `~/.config/opencode/plugin/`, gated on the `// wmux-plugin-version:` marker (`pluginNeedsUpdate` compares it verbatim, so any change to the plugin needs a bump or it reaches nobody — every broken install already has the old file on disk). **That plugin file must export `WmuxPlugin` and nothing else** (#191): OpenCode's auto-discovery loader calls EVERY export as a plugin factory and then invokes a `config` hook on the result, so an exported helper returning a plain value crashes OpenCode at startup. Helpers hang off `WmuxPlugin.__wmuxInternals` for the tests; a source-level test pins the export count. The marker logic itself now lives in `agent-plugin-file.ts` and is re-exported here — there are two of these files since #231, and "is this file ours?" is not a question to answer twice |
| `pi-context.ts` | pi (`@earendil-works/pi-coding-agent`) support (#231). Two halves. **The context block** does NOT always go to `AGENTS.md`: pi loads exactly ONE user-scope context file from `~/.pi/agent`, taking the first that exists of `AGENTS.override.md`, `AGENTS.md`, `AGENTS.MD`, `CLAUDE.md`, `CLAUDE.MD`, and never looking at the rest — so the obvious implementation creates a file, reports success and reaches nobody on a machine that already has an override. `getPiContextFilePath` splices into the file pi will actually read; removal sweeps EVERY candidate, because the target can change between launches and an orphaned block in a file nothing reads is an uninstall that cannot uninstall. **The extension** is what omp-context.ts said was "the right shape for a follow-up": pi auto-discovers `~/.pi/agent/extensions/` GLOBALLY, so unlike omp's per-project `.omp/hooks/` there is no repo to write into and #132 is not engaged. `PI_CODING_AGENT_DIR` is honoured when it is in wmux's own env; a value exported per-shell after launch is unknowable here and falls back to the default — a miss, never a write to the wrong place |
...
| `gpu-watchdog.ts` | The self-heal for a wedged GPU process (#229, 2.10.4). After ~2.5 days the reporter's window kept its last frame while every PTY lived on, `Responding=True`, 0% CPU everywhere; killing ONLY the GPU process restored rendering with the same renderer PID and nothing lost. No repro (hybrid NVIDIA/Intel + two DisplayLink adapters, a 3456×1408 `clear` window). The **detector is a rAF race** in `renderer/utils/gpu-watchdog.ts`: `requestAnimationFrame` runs on BeginFrames from the display compositor in the GPU process, so when it wedges rAF stops while `setTimeout` keeps going — 3 unanswered requests 5 s apart, while `document.visibilityState` is visible, is a report. **The renderer never acts**: main gates on the window being FOCUSED and `powerMonitor.getSystemIdleTime() ≤ 30 s`, because a hidden document is not the only window nobody is looking at (screen off, an always-on-top overlay) and Chromium counts each killed GPU process as a crash — enough of them and it drops to software rendering for the session, hence also 5 min between restarts. `process.kill(pid)` on the `GPU` entry of `app.getAppMetrics()`; Chromium relaunches it on demand and `terminal-renderer.ts`'s context-loss path (#218) repaints every pane. Logs `gpu-stall` (skips, deduped per run), `gpu-restart` and `gpu-gone` to main.log. Off switch: `gpuWatchdog: false` in `wmux-workspace-prefs` |
...
| `session-persistence.ts` | Auto-save/restore window state. `saveSession` is atomic in the sense the comment always claimed and the code did not (#214): it used to `unlink` the live `session.json` before renaming the temp over it, a window with no session file at all — die there and the next launch has nothing to restore, comes up as a fresh Session 1, and re-mints every pane and surface id, which is that issue's "surfaces come back with new ids and lose their customTitle". The unlink was never needed: libuv's `uv_fs_rename` uses `MoveFileExW` with `MOVEFILE_REPLACE_EXISTING`, so a plain `renameSync` over an existing file is legal. It survives only as a fallback for a real sharing violation (antivirus, a sync client), where not saving at all is worse than a brief window. Also the scheduled snapshots (#238): `session.json` holds only the CURRENT layout, so a mistake has nothing behind it — the reporter lost 28 browser panes to a cleanup script and recovered from an older file by luck. `writeSessionSnapshot` keeps a three-slot `Auto-save …` ring on the existing 30s auto-save tick (no second timer — a second one would have a second idea of what the layout is). A ring rather than the single entry that was asked for, because one slot bounds recovery to the state AFTER the loss. `shouldSnapshot` gates on the clock AND on `layoutFingerprint`, which deliberately excludes `cwd`/`bounds`/`sidebarWidth`: those change on every prompt, so including them makes the gate pass every tick, and without the gate an idle machine rotates three identical copies in and deletes its own history |
...
| `zip-updater.ts` | In-place update for portable zip extracts (#184). Detection is the whole contract: `wmux.exe` present, `Uninstall wmux.exe` absent — that name is electron-builder's `Uninstall ${productFilename}.exe`, so it moves if `productName`/`executableName` ever change. Download via `net.request`, extract via System32 `tar.exe` (PowerShell `Expand-Archive` fallback), then a detached cmd helper waits on the old PID and robocopies over the install root. The helper's relaunch is **unconditional** — wmux has already quit, so bailing out on a copy failure is the one outcome the user can't recover from. For the same reason wmux quits only once it knows the helper can run (#3): a missing payload rejects with `PAYLOAD_MISSING` (staged update forgotten, next click downloads), a helper that cannot be written or whose `cmd.exe` emits `'error'` rejects with the payload kept (next click shows the install dialog again — never a straight quit from a badge reading "Click to try again" — and after two failures of the same payload answers `install_failed` with the release page URL), and quit waits for `'spawn'`. The retry keeps the phase at `error` and starts the dialog synchronously: flipping to `ready` first let a second quick click install behind the dialog. `buildHelperArgs` spawns `cmd /d /s /c ""<helper>" "<arg>"…"` verbatim, because Node's per-argument quoting plus `cmd /c`'s strip-first-and-last-quote rule ran `C:\Users\First` on any `%TEMP%` with a space; it also refuses a `%…%` pair, which cmd expands even inside quotes. The helper does **not** strip MOTW and **not** delete itself — hidden `Unblock-File` and a self-erasing script are the two dropper signals behavioural AV reacts to, and neither did anything (the payload has no `:Zone.Identifier`; robocopy replaces the destination's streams; `stripMotw()` covers the rest on launch). The leftovers are removed instead by `sweepUpdateLeftovers`, 60 s after a packaged Windows start whatever the install type, and every rule keeps on doubt: fresh (≤ 1 h, future or unreadable mtime), a zip/extract dir whose PID has a fresh helper beside it (after *Later* the payload is days old while a helper copies from it), a PID `process.kill(pid, 0)` does not answer `ESRCH` for, a symlink/junction, a type that does not match the name. Names are built and parsed in one place (`update*Name` / `classifyUpdateLeftover`). The helper's console is **visible on purpose** and says so in English: `windowsHide` cannot hide it (DETACHED_PROCESS makes Windows ignore CREATE_NO_WINDOW — measured, a detached spawn adds a `conhost.exe` and a non-detached one does not), and the two ways to hide it are both worse — dropping `detached` puts the helper's life back in the hands of a wmux that has already quit, and a hidden detached script overwriting an unsigned exe is the dropper shape the rest of this row is avoiding. It was reported as an empty window titled `findstr.exe`, since Windows renames a console after its current child, which is why `title` is re-set inside the loop. The pause is `waitfor /t N`, never `timeout` (refuses the ignored stdin: `/t 3` returns in 66 ms, so the loop spun `tasklist` at 0.31 s CPU per 8 s wait) and never `ping -n` (the dropper batch-sleep idiom); where `waitfor.exe` is absent it degrades to that old spin |
...
- `Browser/` — BrowserPane, AddressBar, AgentBrowserSetup (the `not-installed` vs `no-dashboard` cards — two genuinely different situations, never one card: the second means the agent browser works fine and only its optional viewer is missing). `utils/browser-start-page.ts` holds the one rule about what a surface opens on when nobody said (#232). A browser surface defaults to **blank**, never to a page on the internet: shipping `https://github.com/amirlehmam/wmux` as `BrowserPane`'s default parameter meant the panel opened the project's own repo on every launch (`openOnStartup` is true by default) — and then the panel's `onUrlChange`, which fires for the INITIAL load too, wrote that default into `workspace.browserUrl` as if a human had picked it. Confirmed on a real install: two workspaces carrying a `browserUrl` nobody typed. GitHub puts "Issues" one click away, so one click pinned wmux's own issue tracker as the start page forever. The read-time filter applies to `workspace.browserUrl` ONLY — that is the one URL wmux opens without being asked; a split-tree browser tab's `surface.url` is written directly by `openInWmuxBrowser` on a link click, and filtering there would blank a pane opened on wmux's issue tracker on purpose. `browserPrefs.defaultUrl` is never filtered: a configured start page is a choice, a remembered one is an inference. The blank state is a themed overlay rather than a bare `about:blank`, which paints WHITE whatever the app theme is
- `Sidebar/` — Sidebar, WorkspaceRow, SessionMenu, SidebarResizeHandle. `workspace-status.ts` holds the THREE chains that must agree about one row — the status words, the status colour, and the state dot — and they have drifted apart twice, which is why they sit in one file beside each other rather than inside the component. The rule that keeps them honest is `mergedAgentSignals` (#235): the agent roster's counts **join** the older hook/observer signals rather than outranking them, so the priority chain stays one chain. Both halves of that matter. A rollup ranked above the tool label coarsens "Reading file…" into "Running…" every time the two agree, and `unknown` — which means "an agent is here and has not said what it is doing" — must fall THROUGH to the heuristics (#128), never over them: as its own priority level it printed the word "Unknown" on a row whose shell knew it was running, and blanked the dot. `unknown` therefore contributes to no count at all, and the invariant is pinned as a PARITY (a silent agent renders byte-identically to no agent) rather than as an expected string — anything a silent agent changes, it changed by overriding a finer signal it knows nothing about
...
**Hooks** (in `hooks/`):
- `useTerminal.ts` — xterm.js lifecycle, PTY connection, OSC notifications, WebGL renderer, OSC 133 prompt marks (#207), touch pan → wheel (#243)
- `utils/wheel-forward.ts` — how many app-level reports one scroll gesture is worth (#245). The two forwarding branches speak **different units**, and conflating them was the bug: a mouse report is a STEP (the app decides how far a step goes — opencode's `scroll_speed`, 3 rows by default) while an arrow key IS one line and nobody scales it. `writeWheelToPty` emitted one report per LINE for both, and a Windows detent is ~100px of `deltaY` against a ~17px cell, so one detent became 5-6 reports and then 15-18 rows — ~5x every other terminal. Users worked around it by dropping `scroll_speed` to 0.25, which fixed the wheel by breaking the touch pan that shares the multiplier. So a mouse-tracking app now gets exactly **one report per wheel EVENT** and the line count is demoted to a gate ("did this cross a line at all?") — which is what xterm does: in `Terminal._bindMouse`, `case 'wheel'`, `consumeWheelEvent()` is only tested against 0 before a single `triggerMouseEvent`. The arrow branch keeps looping per line, and that is a **deliberate divergence** from xterm (which sends one arrow per event there too), not an oversight: wmux's own scrollback branch moves `lines` rows per event, so one arrow per detent would make a pager in an alt-screen pane crawl at a fifth of the speed of the scrollback in the pane beside it. #245 was the multiplier, and the arrow branch has none. **Touch keeps per-line reporting** — a finger is a position, not a detent, and deriving pan speed from a per-event report would tie it to the frame rate; wmux cannot know the app's rows-per-report multiplier, so one report per cell crossed is the closest it can get and is exact wherever that multiplier is 1. The source is carried on the synthetic event by a private `Symbol` in `useTerminal.ts`, NOT by `ev.isTrusted`: trust answers "who dispatched this" rather than "what gesture is this", so the next synthetic wheel from anywhere else would silently inherit touch semantics — and a real-Chromium harness confirms both synthetic events read `isTrusted: false`, so trust is not even observable in a test. Pure, because the count is invisible in a screenshot and the machine that reproduces it is a touchscreen ThinkPad
- `utils/touch-pan.ts` — a finger dragged over a pane scrolled nothing (#243). Upstream: xterm 6.0.0 replaced its native overflow scroller (a finger could drag it) with the VS Code `SmoothScrollableElement`, a wheel-only overlay, and shipped no touch replacement (xtermjs/xterm.js#5489); the fix, #5563, is milestoned 7.0.0 and in no stable release. **A dependency bump would not have been the fix**, and this is the part worth keeping: upstream's repair only moves the scrollable position, which drives SCROLLBACK, and the alternate buffer has none — so a finger would still be inert in exactly the panes wmux exists to show (opencode, Claude Code, Codex all switch to `\x1b[?1049h`). wmux does not use xterm's wheel pipeline at all: `handleTerminalWheel` takes the wheel on the CAPTURE phase and already decides between scrollback, SGR wheel reports and arrow keys for a pager on the alt screen. So the whole fix is to put the gesture into that pipeline as a **synthetic `WheelEvent`** and inherit all three behaviours rather than re-derive any. That is also why this module measures nothing in cells — `wheelDeltaToLines` already keeps a per-surface fractional accumulator against the user's measured cell height, and a second one here would be a second answer to one question. What is left is gesture recognition, and it is pure because the machine that reproduces the bug (a touchscreen ThinkPad) is not the one CI runs on. States mirror the prompt anchor's armed/engaged split for the same reason: the interesting ones are before the feature acts — `tracking` (a finger is down but inside the 8px slop, so it may still be a tap, and a tap must never scroll), `panning`, `rejected` (broke out sideways, or a second finger landed). The axis lock is decided ONCE at slop-crossing and never revisited, or a diagonal drag flickers between scrolling and not. Direction is natural: dragging down reveals what is above, which is a NEGATIVE delta. POINTER events, not Touch events — Electron delivers those reliably while the Touch Events API may not be enabled — and `touch-action: none` on `.terminal-pane__container` is the other half of the fix: with anything else Chromium claims the vertical drag as its own scroll gesture and the pointermove stream stops being cancelable. `pointercancel` must be handled or the tracker keeps a finger down forever and the feature works exactly once. Verified against a real xterm 6.0.0 in real Chromium via a browser harness (the #230 technique), because a capture-phase listener receiving an event dispatched on its OWN element is the load-bearing assumption and no unit test proves it
...
gpu:      reportStall, onRestarted   # GPU watchdog (#229): the renderer reports
                                     # "no frames", main decides and restarts
...
rm -rf ../wmux-release-staging/resources/pi-extension && cp -r resources/pi-extension ../wmux-release-staging/resources/pi-extension               # same failure mode as #149/#81: dev falls back to a repo-relative path, an install has nothing (issue #231)
...
- [ ] opencode-plugin and pi-extension copied to release staging (issues #149, #231) — `npm test` now derives this from `process.resourcesPath` usage in `src/main/`, so a new runtime resource that isn't in `extraResources` fails the release build
...
**DEPRECATED in 2.12.0 (issue #239). wmux no longer installs this, and removes it if a previous version did.**

It never loaded: `ensurePluginRegistered()` wrote Claude Code's `installed_plugins.json` at the top level as an object, while that file is v2 (`{version:2, plugins:{"<plugin>@<marketplace>":[…]}}`) and its cache is `cache/<marketplace>/<plugin>/<version>`, not wmux's `cache/<plugin>/<version>`. Claude Code listed nothing, loaded no skill or command, and wrote `.orphaned_at` through the copied tree — while `enabledPlugins` said `true` and wmux logged an install. The supported repair is a local marketplace; it was not worth doing, because Claude Code now runs parallel agents natively.

The source still ships in `resources/wmux-orchestrator/` — `scripts/launch-agent.js` is the shared agent-command registry that `omp-context`/`pi-context` tests read, and the plugin remains installable standalone from `github.com/amirlehmam/wmux-orchestrator`. Unaffected by the deprecation: the sidebar orchestration panel (`orchestration-watcher.ts` reads a run's `state.json` and does not care who wrote it), `wmux agent spawn`, and the OpenCode plugin, which installs correctly and is what the `orchestrator` consent feature still controls.
...
wmux agent spawn-batch --json '[...]' [--strategy distribute|stack|split] [--workspace W]
                                       # --pane on `spawn` is FULLY specifying since
                                       # 2.13.0 (#242): the workspace is derived from
                                       # the pane that owns it, never from whichever
                                       # one is focused, and naming both when they
                                       # disagree is -32602 rather than a record that
                                       # pairs a pane with a workspace not containing
                                       # it. `spawn-batch` grew --workspace at the same
                                       # time — main honoured a workspace it had no way
                                       # to be told about, and its panes came off the
                                       # ACTIVE workspace regardless.
